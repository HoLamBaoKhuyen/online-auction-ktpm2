...
export default {
    ...
    async updateHighest(entity, proid) {
        console.log(entity);
        return await db('products').where('prodID',proid).update({highestBidID: +entity.highestBidID, curPrice:+entity.curPrice});
    },
    ...
    async resetPrice(id) {
        const sql = `update products p set p.curPrice = p.originalPrice where prodID=` + id + ``;
        return await db.raw(sql);
    },
    async resetHighestBid(id) {
        const sql = `update products p set p.highestBidID = null where prodID=` + id + ``;
        return await db.raw(sql);
    }
}