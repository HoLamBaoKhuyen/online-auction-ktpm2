...
export default {
  ...
  async getParticipatingProd(id) {
    const sql =
      `select p.prodID, p.prodName, p.prodType, p.originalPrice, p.curPrice, p.step, p.highestBidID, p.buyNowPrice, p.timePosted, p.timeEnd, p.selID, p.approve, concat('***** ',u.firstname) AS nameofUser, par.bidID, par.prodID, par.price, par.ptime
                    from products p
                    left join users u on p.highestBidID = u.UID
                    left join participate par on par.prodID = p.prodID
                    where par.bidID = ` +
      id +
      ` group by p.prodID, p.prodName, p.prodType, p.originalPrice, p.curPrice, p.step, p.highestBidID, p.buyNowPrice, p.timePosted, p.timeEnd, p.selID, p.approve, u.firstname, par.bidID, par.prodID, par.price, par.ptime`;
    const raw = await db.raw(sql);
    return raw[0];
  },
  ...
  async countBids(prodID) {
    const sql =
      `select count(par.prodID) AS CountBids
                        from products p
                        left join participate par on par.prodID = p.prodID
                        where par.prodID= ` +
      prodID +
  ...
  async getWinProd(id) {
    const sql =
      `select p.prodID, p.prodName, p.prodType, p.originalPrice, p.curPrice, p.step, p.highestBidID, p.buyNowPrice, p.timePosted, p.timeEnd, p.selID, p.approve, concat('***** ',u.firstname) AS nameofUser, count(par.prodID) AS CountBids
                    ...
                    left join users u on p.highestBidID = u.UID
                    left join participate par on par.prodID = p.prodID
                    where p.highestBidID= ` +
      id +
      ` and p.timeEnd < curdate()
                    group by p.prodID, p.prodName, p.prodType, p.originalPrice, p.curPrice, p.step, p.highestBidID, p.buyNowPrice, p.timePosted, p.timeEnd, p.selID, p.approve, u.firstname;`;
    const raw = await db.raw(sql);
    return raw[0];
  },
  ...
  async getComment(id) {
    const sql =
      `select r.*, concat(u.lastname,' ',u.firstname) AS nameofSeller
                    from rating r
  ...
  async getLikeOfBidder(id) {
    const sql =
      `select count(liked) as likerate
                    from rating r
                    where r.liked = TRUE and r.rateToBidder = TRUE and r.bidID=` +
      id;
    const raw = await db.raw(sql);
  ...
  async getDislikeOfBidder(id) {
    const sql =
      `select count(liked) as dislikerate
                    from rating r
                    where r.liked = FALSE and r.rateToBidder = TRUE and r.bidID=` +
      id;
    const raw = await db.raw(sql);
  ...
  async getLikeOfSeller(id) {
    const sql =
      `select count(liked) as likerate
                    from rating r
                    where r.liked = TRUE and r.rateToBidder = FALSE and r.selID=` +
      id;
    const raw = await db.raw(sql);
    return raw[0][0].likerate;
  },
  ...
  async getDislikeOfSeller(id) {
    const sql =
      `select count(liked) as dislikerate
                    from rating r
                    where r.liked = FALSE and r.rateToBidder = FALSE and r.selID=` +
      id;
    const raw = await db.raw(sql);
    return raw[0][0].dislikerate;
  },
